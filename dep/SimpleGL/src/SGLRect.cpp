...
void SGLRect::setScale(float scale) {
    this->scale = scale;
    transUpdated = true;
}

void SGLRect::setPosition(float x, float y) {
    this->x = x;
    this->y = y;
    transUpdated = true;
}

void SGLRect::setRotation(float rot) {
    this->rotation = rot;
    transUpdated = true;
}

void SGLRect::setColor(float r, float g, float b) {
    if (shader != NULL) {
